				...
				@if ($warning<>'')
				    ...
				    <div class="col-md-5">
				    	<label >Member Details:</label>
				      	<select type="text" id="regno" name="regno" class="form-control"  onchange="ReloadForm()">
					        <option value="">Select Member</option>
					        @foreach ($MemberList as $b)
					        <option value="{{$b->regNo}}" @if(old('regno') == $b->regNo||$regno == $b->regNo) selected @endif>{{$b->Names }} - {{$b->regNo}}</option>
		                	@endforeach 
				 	    </select>
				    </div>
				    <div class="col-md-2">
