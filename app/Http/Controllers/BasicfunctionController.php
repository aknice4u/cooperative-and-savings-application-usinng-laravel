...
class BasicfunctionController extends ParentController
{
	...
	$data= DB::Select("SELECT tbldefaultMonthlyContribution.*,
        CONCAT(tblmembers.`last_name`, ' ', tblmembers.`middle_name`, ' ',tblmembers.`first_name`) as Names,division
        ,(tbldefaultMonthlyContribution.`monthly_contribution`+tbldefaultMonthlyContribution.`loan_repay`+tbldefaultMonthlyContribution.`interest_repay`) as totalPayment
        ,`amount` as payrollcontribution 
        ,`tblmembers`.`branch` as divisionID
        FROM `tbldefaultMonthlyContribution`  
        left join tblmembers on tblmembers.regNo = tbldefaultMonthlyContribution.regno
        left join tbldivision on tbldivision.ID= tblmembers.branch
        left join tblcontributionfromsalary on tblcontributionfromsalary.regno=tbldefaultMonthlyContribution.regno and tblcontributionfromsalary.`month`='$month' and tblcontributionfromsalary.`year`='$year'
	...
	Public function MemberBalanceAsAt ($from,$regno){
		$cbalsum =0;
		$lbalsum =0;
		$ibalsum =0;
		$initdata= DB::Select("SELECT sum(`monthcontribution`) as cbal, sum(`loanborrow`-`loanrepay` ) as lbal, sum(`loaninterest`-`interestrepay`) as ibal 
		...
		return DB::Select("SELECT $cbalsum as 'cbalsum' , $lbalsum as 'lbalsum' ,$ibalsum as 'ibalsum'")[0];
	}
  ...
  Public function MemberList(){
	...
	Public function MemberSummaryLists ($to,$division){
		...
		//die("SELECT $nam,  sum(`monthcontribution`) as CB,
	$data= DB::Select("SELECT tblmembers.regNo , $nam,  sum(`monthcontribution`) as CB,
		`tbldivision`.`division` as Division,
		sum(`loanborrow`-`loanrepay`) as LCB
        ,sum(`loaninterest`-`interestrepay`) as `LIB` 
        FROM `tblmembers`  left join tbltransaction on tbltransaction.regno=tblmembers.regNo  
        left join `tbldivision` on `tbldivision`.`ID`=`tblmembers`.`branch`
        WHERE  $qdivision  and DATE_FORMAT(`transaction_date`,'%Y-%m-%d')<='$to' group by tblmembers.regNo");
		//dd($data);
		return $data;
	...
	$data= DB::Select("SELECT tblcontributionfromsalary.*,CONCAT(`last_name`, ' ', `middle_name`, ' ',`first_name`) as Names,division, tbldivision.ID as divid 
